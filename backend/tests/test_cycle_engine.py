...
def test_preset_library_registers_required_strength_templates() -> None:
    ...
    assert presets["madcow_5x5"].supportedWeeks == [1, 2, 3, 4, 5]
    assert presets["texas_method"].supportedWeeks == [1]
...
def test_build_cycle_week_plan_for_madcow_prefers_tm_for_percentage_movements() -> None:
    ...
    assert monday_squat["instance"]["sourceMax"] == 162
    assert monday_squat["instance"]["targetWeight"] == 129.6
    assert friday_top_set["pct"] == 0.9
    assert friday_top_set["instance"]["sourceMax"] == 162
    assert friday_top_set["instance"]["targetWeight"] == 145.8
