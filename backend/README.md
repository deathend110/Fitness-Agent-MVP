...
- Gemini `functionDeclarations` 校验比 OpenAI/DeepSeek 严格：拒绝无 `type` 的属性 schema，也拒绝 `properties` 为空的 object。`propose_plan_change.changes[].newValue`（`Any` 类型）在 Gemini 适配层兜底为 `{"type":"string"}`，下游 `_coerce_number` 仍会把 `"0.7"` 这类数值字符串解析回数字；`propose_day_plan_replace.dayPlan` 已定义为结构化的 `DayPlanArgs`（含 `type` 与 `exercises`，`exercises` 进入 nested required），避免空属性 object 被 Gemini 拒绝，同时给 DeepSeek 更明确的结构指引
