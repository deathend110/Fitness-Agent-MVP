...
function SectionCard({ children, eyebrow, title, description }) {
  return (
    <section className="rounded-[1.75rem] border border-fitloop-line bg-fitloop-panel p-6 shadow-xl shadow-black/20 sm:p-7">
...
function ProfileTab({
  ...
  const summaryCards = buildProfileSummaryCards(profile)

  return (
    <div className="space-y-6">
      <section className="overflow-hidden rounded-[2rem] border border-fitloop-line bg-fitloop-panel/90 shadow-2xl shadow-black/20">
        <div className="relative px-6 py-8 sm:px-8 sm:py-9">
          <div className="pointer-events-none absolute inset-x-0 top-0 h-32 bg-[radial-gradient(circle_at_top_left,rgba(109,94,252,0.16),transparent_58%)]" />
          <div className="relative">
            ...
            <div className="mt-4 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              ...
              <div className="inline-flex items-center rounded-full border border-fitloop-orange/30 bg-fitloop-orange/8 px-4 py-2 text-sm text-fitloop-orange">
                当前数据会同步保存到本地与后端
              </div>
            </div>
          </div>
          ...
          <div className="relative mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {summaryCards.map((card) => (
              <article
                className="rounded-[1.5rem] border border-fitloop-line bg-white/80 p-5 shadow-lg shadow-black/20 backdrop-blur"
                key={card.key}
              >
