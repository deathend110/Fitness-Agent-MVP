function PlanWeekGridColumn({ children, column }) {
  const baseClassName = 'min-w-0 h-full rounded-[1.25rem] border shadow-sm transition'
  ...
  return (
    <article
      className={`${baseClassName} flex flex-col ${column.isTrainingDay ? trainingClassName : restClassName}`}
    >
      {children}
    </article>
  )
