...
- Phase 4 Task 26 已新增安全 Markdown 渲染与消息列表自动滚动到底，并补齐表格与分割线渲染
...
- V2.4 已新增消息级附件快照：用户发送带附件的问题后，附件会绑定到该条用户消息，刷新页面和切换历史会话后仍可回显
- AI 教练后台思考已补齐窗口 blur/focus 与组件卸载接续，后台任务也复用 Tool Calls 计划 proposal 链路，保证切页后生成的采纳卡片仍可提交
- AI 教练历史侧栏已接入后端真实会话列表，新建对话会创建新的 `chat_session`，切换会话会从后端拉取对应消息和草稿
...
- AI 回复支持安全 Markdown 展示，标题、列表、代码块、表格和分割线可正常渲染，新消息/流式输出会自动滚动到最新
- AI 输入草稿、模型、thinking 和附件 id 可保存到后端草稿
- AI 教练已发送的用户消息会显示附件卡片，包含文件图标、文件名和类型/大小；即使输入区附件已清空，历史消息仍保留该快照
- AI 教练左侧历史记录来自后端真实会话；刷新后会恢复最近使用的会话，点击新建对话不会删除旧对话
...
- 历史侧栏展示项来自后端 `GET /api/chat/sessions`，点击会话后通过 `GET /api/chat/sessions/{id}/messages` 恢复该会话消息。
- 采纳卡片的展示模型与领域建议对象已分离：`suggestionCard` 仅负责展示，采纳 / 忽略回调继续传递原始 `suggestion`。
- 顶部已移除“上下文”按钮；“新建对话”会创建新的后端会话，旧会话继续保留在左侧历史记录中。
...
`tests/e2e/coach_session_history.py` 会额外验证真实会话历史：侧栏展示后端会话、切换会话恢复对应消息、新建对话后旧会话仍留在 UI 中。
...
9. 切换到“AI 教练”，确认顶部不再出现“上下文”按钮，侧栏显示后端真实会话列表。
...
18. 发送成功后，确认该条用户消息正文上方出现附件卡片，显示文件图标、文件名与类型/大小，而不是只留在输入区 chip。
19. 刷新页面或切换到其他历史会话再切回，确认刚才那条用户消息的附件卡片仍可回显。
20. 切换模型和 thinking 配置后发送，确认后端聊天请求携带对应 `model/thinking`。
21. 输入未发送草稿并刷新页面，确认草稿、模型配置和附件 id 从后端恢复；发送成功后草稿与附件清空。
22. 让 AI 回复包含标题、列表、代码块、Markdown 表格或 `---` 分割线，确认消息都能按安全 Markdown 正常渲染并自动滚动到最新。
...
- `fitloop_weeklyPlan`
- `fitloop_dailyLog`
- `fitloop_chatHistory`（兼容导入导出与当前会话展示缓存；AI 教练历史主数据已切到后端 chat 表）
- `fitloop:coach-active-session-id`（保存 AI 教练页当前选中的真实后端会话 id）
