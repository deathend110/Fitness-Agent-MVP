...
- 后端：FastAPI + SQLite（V2.3 Phase 1 已接入，Phase 2 Task 8 已补充聊天存储接口）
- 存储：
  - `profile / weeklyPlan / dailyLog` 走本地 Python 后端
  - `chat` 会话与消息已具备后端 SQLite 存储接口
  - AI 教练页前端仍暂用浏览器 `localStorage`，将在后续 Task 9 切到后端聊天接口
...
- AI 教练页前端尚未切换到后端 chat 接口
- DeepSeek 后端代理 API 与 SSE 流式
- 后台思考
- 计划采纳后端化
...
- AI 教练页前端旧直连路径仍会读取项目根目录的 `VITE_DEEPSEEK_API_KEY`，该路径将在 Task 9 移除

如需运行当前前端 AI 教练页，在项目根目录创建 `.env`：
...
说明：这些接口当前只完成后端存储能力；AI 教练页前端切源和 SSE 流式会在后续 Task 9 接入。
...
- `fitloop_weeklyPlan`
- `fitloop_dailyLog`
- `fitloop_chatHistory`（AI 教练页前端当前仍使用，后续会迁到后端 chat 接口）
