...
- 后端：FastAPI + SQLite（V2.3 Phase 1 已接入，Phase 2 Task 9 已补充聊天存储与 SSE 代理）
- 存储：
  - `profile / weeklyPlan / dailyLog` 走本地 Python 后端
  - `chat` 会话与消息已具备后端 SQLite 存储接口
  - AI 教练页发送消息已切到后端 `/api/chat/stream`，失败时回退 `/api/chat/reply`
...
说明：Task 9 的“瘦聊天代理”仍由前端构建 system prompt 并传入 `messages[]`；后端负责藏密钥、流式转发、解析 `suggestion` 并在完整成功后落库 user + assistant。后台思考和计划采纳后端化仍留给后续任务。
