...
- 后端：FastAPI + SQLite（V2.3 Phase 1 已接入，Phase 2 已补充聊天存储、SSE 代理、离页后台思考与计划采纳校验）
- 存储：
...
- prompt / 指标 / 上下文管理仍由前端构建，后端上下文压缩与工具调用留后续阶段
- 文件上传、模型选择、知识库、周期计划与预制计划库仍未实现

V2.3 当前重点已经完成：密钥后移、聊天全量落库、SSE 流式代理、离页后台思考和计划采纳后端校验。
