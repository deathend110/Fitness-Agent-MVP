...
- 后端：FastAPI + SQLite（V2.3 Phase 1 已接入，Phase 2 已补充聊天存储、SSE 代理、离页后台思考与计划采纳校验；Phase 3 已开始接入后端 Agent 上下文编排）
- 存储：
...
当前仓库已完成 Python 后端化 Phase 1 + Phase 2：
...
当前仍未完成前后端闭环的内容将进入后续 V2.5 / Phase 3+：

- AI 教练页真实多会话 UI 尚未完全接入后端会话列表
- 前端正式切到只传 `userInput/sessionId` 的新 Agent 请求契约仍在后续 Task 20；当前后端已兼容新旧两种请求
- 后端长对话摘要压缩、长期 memory 提取、usage 记录和工具调用循环留后续 Phase 3 task
- 文件上传、模型选择、知识库、周期计划与预制计划库仍未实现

V2.3 当前重点已经完成：密钥后移、聊天全量落库、SSE 流式代理、离页后台思考和计划采纳后端校验；Phase 3 已开始把 prompt 和上下文管理迁入后端 Agent Orchestrator。
...
11. 输入训练调整问题并发送，观察聊天消息、SSE 流式回复、非流式回退与错误提示。
12. 发送后立刻切到其他标签页，再回到“AI 教练”，确认后台思考完成结果能补回当前对话；如果当前对话已清空，应只提示不污染新对话。
13. 当 AI 回复包含 `---JSON---` 结构化建议时，确认页面在对应 AI 消息下方渲染采纳卡片。
14. 点击“采纳并更新计划”，返回“训练计划”确认对应动作字段已通过后端 `/api/weekly-plan/adopt` 更新并持久化。
...
说明：Phase 2 旧请求仍可传入完整 `messages[]`；Phase 3 新请求可只传 `sessionId + userInput + model`，由后端 `chat_session.build_agent_request()` 读取 SQLite 中的档案、周计划、日志、会话摘要、长期记忆和最近消息并拼装 DeepSeek messages。当前前端正式切换留给 Task 20，因此后端同时兼容新旧路径。离页后台思考会先中止当前前台请求，只有成功拿到 `task_id` 后才抑制前台错误；回页补结果时会校验源 user 消息的本地下标与内容仍匹配，避免旧后台结果污染已清空或同文本重开的新对话。后台思考使用进程内任务表，服务重启后旧 task_id 不再可查；计划采纳仍保留用户点击确认闸门，AI 建议不会自动写回。
