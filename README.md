...
- AI 教练页在发送中切到其他 RepMind 页签再切回时，会继续保留最后一条 user 气泡和“思考中”占位，直到真实回复完成或后台任务状态变化
- AI 教练会在后端自动读取档案、周计划、今日日志、近期对话、会话摘要、记忆和文件摘要
- 当模型返回结构化计划建议时，前端会展示待确认的计划卡，用户可确认写回或忽略
- `day_plan_replace` 类型的新计划卡会在后端统一补齐重量字段语义：整块负重缺失时可沿用唯一同名动作的原负重；只缺 `ref1RM/loadMode` 这类来源字段时只补来源、不偷带旧 `pct/kg`；若同一动作同时给出百分比与固定重量信号，后端会按显式负重模式保留一侧并把冲突写入备注
...
```powershell
uv run python tests\e2e\coach_tab_pending_navigation_restore.py
```
