...
- 后端已内置周期计划预制模板库与确定性周生成引擎，当前支持 `candito_6week`、`madcow_5x5`、`texas_method` 三类力量模板，可生成兼容现有 `weeklyPlan` 的单周计划并合并单周 override
...
```powershell
uv run pytest backend\tests\test_cycle_engine.py -q
```
