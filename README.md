...
- 当前已完成的核心能力：
  ...
  - AI 返回合法建议 JSON 时渲染采纳卡片，并支持一键采纳写回周计划
...
15. 点击“忽略”，确认卡片消失。
16. 点击“采纳并更新计划”，确认页面出现“已采纳 AI 建议，训练计划已更新。”提示。
17. 回到“训练计划”，确认对应日期和动作的字段已经同步更新。
18. 用一个不存在的 `exerciseName` 建议做验证，确认页面显示失败提示且训练计划不被修改。
...
- 新增 `src/utils/adoptPlan.js`，统一处理 suggestion 的 `day / changes` 到 `weeklyPlan` 的写回逻辑。
- `src/App.jsx` 会把 `onWeeklyPlanChange` 继续传给 `CoachTab`，采纳成功后仍通过顶层 state 与 `useEffect` 写回 `fitloop_weeklyPlan`。
- `src/tabs/CoachTab.jsx` 在采纳成功时会清除当前卡片并给出成功提示；找不到目标日期、动作或字段时会保留卡片并显示失败提示。
- 当前 MVP 只处理 suggestion 中已有动作的字段更新，不扩展到新增/删除动作。

## Task 4.6 测试命令

```bash
node --test tests/adoptPlan.test.js tests/adoptCard.test.js tests/weeklyPlan.test.js
```
