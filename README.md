...
- 后端：FastAPI + SQLite
- AI 接口：当前默认以运行时 `modelRef` 驱动 OpenAI-compatible 与 Gemini-native 双协议；Gemini 继续直连原生运行时，DeepSeek 现默认也按 OpenAI-compatible `/v1` 链路接入，并会根据 provider 的 `wireApi + apiPathMode` 在 `chat_completions` 与 `responses` 两条 wire 之间选择正确客户端，不再错误回退到旧 DeepSeek 直连链路
