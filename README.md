...
- 后端：FastAPI + SQLite（V2.3 Phase 1 已接入，Phase 2 已补充聊天存储、SSE 代理、离页后台思考与计划采纳校验；Phase 3 已完成后端 Agent Orchestrator、上下文压缩、memory、usage 观测与工具调用）
- 存储：
...
当前仓库已完成 Python 后端化 Phase 1 + Phase 2 + Phase 3：
...
- 文件上传解析、模型选择、前端 Markdown 渲染、后端草稿持久化、真实多会话 UI、周期计划与预制计划库仍未实现
- 当前 `read_uploaded_file_summary` 仍是预留工具，等待 Phase 4 文件上传解析接入后补全真实文件摘要

V2.3 当前重点已经完成：密钥后移、聊天全量落库、SSE 流式代理、离页后台思考、计划采纳后端校验，以及 Phase 3 后端 Agent Orchestrator。前端不再负责拼装 system prompt，后端统一管理上下文窗口、摘要、memory、工具调用和用户确认写回。
...
14. 点击“采纳并更新计划”，返回“训练计划”确认对应动作字段已通过后端 `/api/tools/plan/commit` 或兼容 `/api/weekly-plan/adopt` 更新并持久化。
