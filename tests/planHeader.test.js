...
test('buildPlanHeaderModel 会生成训练计划头部展示模型', () => {
  ...
  assert.equal(headerModel.viewTabs, undefined)
...
test('PlanHeaderToolbar 源码恢复效果稿的一行式结构与计划设置入口', () => {
  ...
  assert.doesNotMatch(toolbarSource, /headerModel\.viewTabs\.map/)
  assert.doesNotMatch(toolbarSource, /周视图/)
  assert.doesNotMatch(toolbarSource, /列表视图/)
