...
test('buildDailyMetricsPanelModel 会把计划类型和状态色整理成可直接展示的中文与样式 token', () => {
  ...
  const successTone = getMetricToneClassNames('success')
  const neutralTone = getMetricToneClassNames()

  assert.equal(panelModel.header.planTypeLabel, '未设置')
  assert.equal(warningTone.labelClassName, 'text-amber-700')
  assert.equal(warningTone.valueClassName, 'text-amber-800')
  assert.equal(successTone.labelClassName, 'text-emerald-700')
  assert.equal(successTone.valueClassName, 'text-emerald-800')
