...
test('App 会为保活的 CoachTab 容器继续传递 h-full 与 min-h-0，避免顶部和底部操作区被挤出视口', () => {
  const source = readFileSync('src/App.jsx', 'utf-8')

  assert.match(source, /aria-hidden=\{activeTab\.id !== 'coach'\}/)
  assert.match(
    source,
