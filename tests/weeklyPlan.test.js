...
test('updateDayType 将某天改成 rest 时保留原有动作', () => {
  const nextPlan = updateDayType(defaultWeeklyPlan, 'Monday', 'rest')

  assert.equal(nextPlan.Monday.type, 'rest')
  assert.equal(nextPlan.Monday.exercises.length, defaultWeeklyPlan.Monday.exercises.length)
  assert.deepEqual(nextPlan.Tuesday, defaultWeeklyPlan.Tuesday)
})

test('addExerciseToDay 新增百分比动作时会生成 id 且 kg 为 null', () => {
  const nextPlan = addExerciseToDay(defaultWeeklyPlan, 'Tuesday', {
    name: '深蹲',
    ref1RM: 'squat',
    pct: 0.75,
    kg: null,
...
test('updateExerciseInDay 只更新目标日期中的目标动作', () => {
  const nextPlan = updateExerciseInDay(defaultWeeklyPlan, 'Monday', 'monday-squat', {
    id: 'monday-squat',
    name: '暂停深蹲',
    ref1RM: 'squat',
    pct: 0.7,
  ...
  assert.equal(nextPlan.Wednesday.exercises[0].name, defaultWeeklyPlan.Wednesday.exercises[0].name)
})

test('removeExerciseFromDay 删除动作时不会影响其他日期', () => {
  const nextPlan = removeExerciseFromDay(defaultWeeklyPlan, 'Monday', 'monday-rdl')

  assert.equal(nextPlan.Monday.exercises.some((exercise) => exercise.id === 'monday-rdl'), false)
  assert.equal(nextPlan.Monday.exercises.length, defaultWeeklyPlan.Monday.exercises.length - 1)
  assert.equal(nextPlan.Friday.exercises.length, defaultWeeklyPlan.Friday.exercises.length)
})
