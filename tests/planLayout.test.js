...
test('PlanDayCard 源码会按动作数和编辑态决定是否允许动作拖拽', () => {
  ...
  assert.match(cardSource, /const\s+isEditingDay\s*=\s*editingExerciseId\s*!==\s*null/)
  assert.match(
    cardSource,
    /canReorderPlanDayExercises\(\{\s*editingState:\s*\{\s*dayKey:\s*isEditingDay\s*\?\s*dayKey\s*:\s*null,\s*exerciseId:\s*editingExerciseId,\s*\},\s*dayKey,\s*exerciseCount:\s*plan\.exercises\.length,\s*\}\)/s,
  )
  assert.match(cardSource, /dragEnabled=\{dragEnabled\}/)
  assert.match(cardSource, /onMoveExercise=\{onMoveExercise\}/)
  assert.match(cardSource, /&&\s*editingExerciseId\s*!==\s*NEW_PLAN_EXERCISE_ID/)
})
