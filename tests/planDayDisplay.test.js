...
test('buildPlanDayDisplayModel 不会把 manual_strength 这类原始训练类型字段直接暴露到页面文案', () => {
  const model = buildPlanDayDisplayModel({
    dayLabel: '周一',
    plan: {
      type: 'manual_strength',
      exercises: [{ id: 'manual-squat', name: 'Manual Squat' }],
    },
    isTrainingDay: true,
  })

  assert.equal(model.preview.title, '训练日')
})

test('训练日头部不再渲染腿日等训练类型胶囊', () => {
  const model = buildPlanDayDisplayModel({
    dayLabel: '周一',
    plan: {
      type: '腿日',
      exercises: [{ id: 'squat', name: '深蹲' }],
    },
    isTrainingDay: true,
  })
  const headerSource = readWorkspaceFile('src/components/PlanDayCardHeader.jsx')

  assert.equal(model.headerBadgeLabel, null)
  assert.doesNotMatch(headerSource, /: planType\)/)
  assert.match(headerSource, /const shouldShowTypeBadge = Boolean\(typeLabel\)/)
})
...
test('PlanDayCardHeader 不再为训练日单独渲染“训练日”这一行文案', () => {
  const headerSource = readWorkspaceFile('src/components/PlanDayCardHeader.jsx')

  assert.doesNotMatch(headerSource, /{isTrainingDay \? preview\.title :/)
})
