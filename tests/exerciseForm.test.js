...
test('getRpeValidationError 会复用共享 guardrail 拦截范围与步长非法值', () => {
  assert.equal(typeof exerciseForm.getRpeValidationError, 'function')
  assert.equal(exerciseForm.getRpeValidationError('11'), 'RPE 必须在 0-10/10 之间')
  assert.equal(exerciseForm.getRpeValidationError('-1'), 'RPE 必须在 0-10/10 之间')
  assert.equal(exerciseForm.getRpeValidationError('8.3'), 'RPE 必须按 0.5 的步长填写')
...
test('buildExerciseSavePayload 会统一拦截非法固定重量、组数、次数和百分比', () => {
  assert.equal(
    exerciseForm.buildExerciseSavePayload({
      name: '深蹲',
      weightMode: 'fixed',
...
test('getRpeFieldHint 会给出清晰的范围提示，并在出错时优先显示错误', () => {
  ...
  assert.equal(
    exerciseForm.getRpeFieldHint('RPE 必须按 0.5 的步长填写'),
    'RPE 必须按 0.5 的步长填写',
  )
})
