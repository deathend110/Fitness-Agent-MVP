...
test('createBackendClient 会使用 JSON POST/PUT/GET 调用约定接口', async () => {
  ...
  assert.equal(requests[10].url, 'http://127.0.0.1:8000/api/chat/sessions/12/messages')
  assert.equal(requests[10].options.method, 'GET')
  assert.equal(requests[11].url, 'http://127.0.0.1:8000/api/files/99')
  assert.equal(requests[12].url, 'http://127.0.0.1:8000/api/chat/sessions/12/draft')
  assert.equal(requests[13].url, 'http://127.0.0.1:8000/api/chat/sessions/12/draft')
  assert.equal(requests[13].options.method, 'PUT')
  assert.deepEqual(JSON.parse(requests[13].options.body), { content: 'hello', attachedFileIds: [1] })
  assert.equal(requests[14].url, 'http://127.0.0.1:8000/api/metrics/daily-summary?date=2026-06-01')
})
