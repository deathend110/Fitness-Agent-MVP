...
test('PlanSettingsPanel 会挂载自定义力量编辑器入口与创建动作文案', () => {
  ...
  assert.match(source, /CustomStrengthPlanDialog/)
  assert.match(source, /打开自定义力量周期计划弹窗/)
  assert.match(source, /isCustomStrengthDialogOpen/)
...
test('CustomStrengthPlanDialog 源码会复用 AI 设置弹窗模式承载自定义力量编辑器', () => {
  const source = readFileSync(
    'src/components/plan-settings/CustomStrengthPlanDialog.jsx',
    'utf-8',
  )
