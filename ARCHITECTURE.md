...
- 当前正式流式计划卡链路以 OpenAI-compatible provider runtime 为中心：先工具回环拿 proposal，再流式正文，正文后才发 proposal 卡，最后 done
...
- 本地 FastAPI 后端已提供 `chat_session / chat_message` 的存储接口、`POST /api/chat/stream` SSE 代理、离页后台思考任务和计划采纳校验接口
...
- 模型运行时超时现统一收口到 `LLM_TIMEOUT_SECONDS`；旧配置名 `DEEPSEEK_TIMEOUT_SECONDS` 仍兼容并会映射到同一入口，避免 DeepSeek / OpenAI-compatible / Gemini / 后台任务各自持有不同超时口径
...
- `backend/api/chat.py`
  ...
  - `POST /api/chat/stream` 现在直接接收 JSON body，不再依赖 query string 传长文本、thinking 或 fileIds
...
- 通过 `backend/agent/chat_session.py` 里的共享 provider runtime 选择器，前台聊天与后台任务会使用同一套 provider-bound client：Gemini-native 继续直连 `GeminiNativeClient`，OpenAI-compatible 会按 `wireApi/apiPathMode` 选择 `chat_completions` 或 `responses` 运行时；DeepSeek 默认也绑定到这套 OpenAI-compatible `/v1` 运行时，不再错误回退到旧直连链路
  - `/api/chat/stream` 当前稳定事件顺序只承诺两种：普通回复 `delta* -> suggestion -> done`；计划卡回复 `delta* -> proposal -> suggestion -> done`
  - 计划卡场景下会先完成 proposal 工具回环，再开始流式正文；proposal 事件只会在正文流结束并完成持久化准备后发出，`done` 必须最后发出
  - 前端解析层仍兼容 `tool_status` 事件，但当前后端正式主链路不把它作为稳定时序保证
...
- `src/api/coachBackend.js`
  ...
  - 解析后端 SSE 帧并把 `delta / suggestion / proposal / tool_status / done / error` 映射给页面；其中当前正式主链路重点依赖 `delta / proposal / suggestion / done`
...
CoachTab
  ...
  -> run_tool_calling_chat()
      -> provider runtime tools 只读读取档案、计划、日志、指标和 memory
      -> `propose_plan_change / propose_day_plan_replace` 生成需确认 proposal
      -> 若为计划卡请求，先停在 proposal ready
  -> provider runtime 流式正文
      -> 先连续返回 delta
      -> 正文完成后返回 proposal / suggestion
      -> 最后返回 done
  -> 渲染文本回复与 AdoptCard
  -> 用户点击采纳
...
后端 AI 代理能力
  -> POST /api/chat/stream {sessionId?, userInput|messages, model?, thinking?, fileIds?}
      -> build_agent_request()
      -> 若用户明确要求计划卡：
          -> run_tool_calling_chat() 先拿 proposal
          -> provider runtime 开始流式正文
          -> event: delta*
  ...
  -> POST /api/chat/reply
      ...
      -> provider runtime request_chat_with_usage()，必要时执行工具循环
...
CoachTab
  ...
  -> GET /api/models
      -> ProviderRuntimeCache.list_enabled_models()
      -> 返回当前启用模型、默认 modelRef 与兼容顶层 thinking
...
错误边界：SSE 中任意 provider runtime 凭据缺失、上游错误或断流都会转成 `event: error`；后端不会写入半截 assistant。前端收到流式错误后会尝试 `/api/chat/reply` 非流式回退。工具循环超过 4 轮、工具参数非法、proposal 重复提交或过期，都会返回可解释错误并保留原计划。
OpenAI-compatible `responses` 若遇到 SSE 上游抖动、`502/503/504` 或 `function_call_output` HTTP 续传受限，会自动降级到 `chat_completions`；但稳定约束仍是“不额外先打一轮假的非流式正文、不提前发 proposal 卡、done 始终最后”。
...
Phase 4 文件上传中，空文件返回 422、不支持扩展名返回 415、超过大小限制返回 413；解析失败不会删除记录，也不会阻断普通聊天。draft 保存遇到不存在的附件 id 返回 422。
...
CoachTab
  ...
  -> POST /api/chat/stream 或 POST /api/chat/reply
  -> build_agent_request()
      -> PromptAssembler
  ...
  -> OpenAI-compatible provider runtime
      -> 普通回复：直接流式正文
      -> 计划卡回复：先工具拿 proposal，再流式正文，再发 proposal / suggestion / done
  -> parse_ai_response(content)
...
- 当前正式稳定边界只承诺 OpenAI-compatible 主链路的 `delta* -> suggestion -> done` 与 `delta* -> proposal -> suggestion -> done` 两种顺序；Gemini、legacy 与 `tool_status` 仍保留兼容，但不作为这次主链路改造的同等级保证
- 计划卡流式场景必须满足“先工具、再流式正文、正文后出卡、最后 done”；前端也只在收到 `proposal` 事件后才挂出 proposal 卡
